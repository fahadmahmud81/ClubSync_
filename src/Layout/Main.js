...
const Main = () => {
    ...
    const pathsWithoutHeader = ['/login', '/register', "/student_panel", '/success_payment', '/fail_payment', '/certificatePdf/:id'];
    const showHeader = !pathsWithoutHeader.includes(location.pathname) && !location.pathname.startsWith('/certificatePdf/');
    const pathsWithoutFooter = ['/login', '/register', "/student_panel", '/success_payment', '/fail_payment', '/certificatePdf/:id'];
